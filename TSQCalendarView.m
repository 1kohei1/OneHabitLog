...
{
    TSQCalendarMonthHeaderCell *cell = [[[self headerCellClass] alloc] initWithCalendar:self.calendar reuseIdentifier:identifier habit_id:0];
...
- (void)refresh:(ODRefreshControl *)refresh {
    ...
    [self.delegate calendarView:self didScroll:0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    if (indexPath.row == 0 && !self.pinsHeaderToTop) {
        ...
        if (!cell) {
            cell = [[[self rowCellClass] alloc] initWithCalendar:self.calendar reuseIdentifier:identifier habit_id:_habit_id];
            cell.backgroundColor = self.backgroundColor;
            cell.calendarView = self;
        }
        return cell;
